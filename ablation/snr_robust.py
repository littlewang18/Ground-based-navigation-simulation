...
def _make_run_df() -> pd.DataFrame:
    ...
    for snr_idx, snr_db in enumerate(_SNR_GRID):
        cfg_snr = replace(cfg, rx=replace(cfg.rx, snr_db=float(snr_db)))
        for rep_ord, rep_idx in enumerate(_rep_idx_lst(cfg), start=1):
            cfg_rep = _make_rep_cfg(cfg_snr, rep_idx)
            cfg_run, meta = _jitter_mp(cfg_rep, snr_idx, rep_idx)
...
def main() -> None:
    ...
    summary_df.to_csv(out_dir / "snr_robust.csv", index=False, encoding="utf-8-sig")
    case_df.to_csv(out_dir / "snr_robust_case.csv", index=False, encoding="utf-8-sig")
    with pd.ExcelWriter(out_dir / "snr_robust.xlsx") as xw:
    ...
    print(f"\npng: {png}")
    print(f"xlsx: {out_dir / 'snr_robust.xlsx'}")
