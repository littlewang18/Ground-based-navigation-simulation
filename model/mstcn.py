...
from config import Cfg, MsTcnCfg
...
class MsTcnData:
    ...
    q_pred: np.ndarray  # 多尺度TCN 预测质量 (n_ep, n_bs)
...
def _build_seq(res: np.ndarray, seq_n: int) -> np.ndarray:
    """???????"""
    return build_seq4(res, seq_n, "mstcn.seq_n")


def _prep_data(obs: ObsData, geo: GeoData, cfg: Cfg) -> MsTcnPrepData:
    """构造 多尺度TCN 的输入与监督。"""
    ...
    seq = _build_seq(mp_hat, cfg.mstcn.seq_n)
    q_tar = make_pos_q(cfg, geo, rho_hat, rho_true, obs.t_ep, cfg.mstcn.q_scale)
    return MsTcnPrepData(
        rho_true=rho_true,
        mp_raw=mp_raw,
        rho_hat=rho_hat,
        mp_hat=mp_hat,
...
def _make_mc_cfg(cfg: Cfg, idx: int) -> Cfg:
    """???? Monte Carlo ?????"""
    return make_mc_cfg(cfg, cfg.mstcn, idx)


def _build_mc_train(cfg: Cfg) -> tuple[np.ndarray, np.ndarray]:
    """?? Monte Carlo ????"""
    return build_mc_train(cfg, cfg.mstcn, _prep_data)
...
class MsTemporalBlock(nn.Module):
    ...
    def __init__(self, in_n: int, out_n: int, ker_lst: tuple[int, ...], dil: int) -> None:
        ...
        self.fuse = nn.Conv1d(out_n * len(ker_lst), out_n, 1)
        self.proj = nn.Conv1d(in_n, out_n, 1) if in_n != out_n else nn.Identity()

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """前向计算。"""
        y_lst = [branch(x) for branch in self.branches]
...
class MsTcnNet(nn.Module):
    """轻量多尺度 TCN 质量预测网络。"""

    def __init__(self, in_n: int, hid_n: int, layer_n: int, ker_lst: tuple[int, ...]) -> None:
        super().__init__()
        mods = []
        c_in = in_n
        for i in range(layer_n):
            mods.append(MsTemporalBlock(c_in, hid_n, ker_lst, dil=2**i))
            c_in = hid_n
...
def _eval_val_d3(model: MsTcnNet, eval_pack: Any, cfg: MsTcnCfg, dev_cfg: Any) -> float:
...
) -> tuple[MsTcnNet, np.ndarray, np.ndarray]:
    """训练 Torch 版多尺度 TCN。"""
    if torch is None or nn is None or F is None:
        raise RuntimeError("当前环境缺少 PyTorch，无法运行 Torch 版 多尺度TCN")
    if len(cfg.ker_lst) == 0:
        raise ValueError("mstcn.ker_lst 不能为空")

    dev = pick_dev(dev_cfg, "mstcn")
    seed_torch(cfg.seed, dev)
    model = MsTcnNet(in_n=seq.shape[-1], hid_n=cfg.hid_n, layer_n=cfg.layer_n, ker_lst=cfg.ker_lst).to(dev)
    x = to_f32(seq, dev)
...
def _pred_torch(model: MsTcnNet, seq: np.ndarray, dev_cfg: Any) -> np.ndarray:
    """Torch 模型推理。"""
    dev = pick_dev(dev_cfg, "mstcn")
    x = to_f32(seq, dev)
    model.eval()
...
def _pred_rule(seq: np.ndarray, cfg: MsTcnCfg) -> np.ndarray:
    """Torch 不可用时的规则回退。"""
    last = seq[:, -1, 0]
    rms = seq[:, -1, 3]
...
def _make_w(q_pred: np.ndarray, cfg: MsTcnCfg, n_ep: int, n_bs: int) -> np.ndarray:
    """?????????????"""
...
def run_mstcn(cfg: Cfg, obs: ObsData, geo: GeoData) -> MsTcnData:
    """运行 多尺度TCN 模型并输出加权定位结果。"""
    ...
    eval_pack = build_q_eval_case(cfg, cfg.mstcn, obs, geo, prep)

    n_ep, n_bs = prep.mp_hat.shape
    seq_eval = prep.seq.reshape(n_ep * n_bs, cfg.mstcn.seq_n, prep.seq.shape[-1])
    q_pred, loss, val_d3 = _fit_predict(seq_train, q_train, seq_eval, cfg.mstcn, cfg.dev, eval_pack)
    q_pred = np.clip(q_pred, 0.0, 1.0)
    w = _make_w(q_pred, cfg.mstcn, n_ep, n_bs)
    ...
    return MsTcnData(
        ...
        loss=loss,
        val_d3=val_d3,
        mc_n=int(cfg.mstcn.mc_n),
        train_sample_n=int(seq_train.shape[0]),
    )


def mstcn_stat(data: MsTcnData) -> Dict[str, Any]:
    """输出多尺度TCN 摘要。"""
    return {
        "dim": int(data.dim),
...
def save_mstcn(data: MsTcnData, out_dir: Path) -> Path:
    """保存多尺度TCN 结果。"""
    out_dir.mkdir(parents=True, exist_ok=True)
    out = out_dir / "mstcn.npz"
    np.savez(
        out,
        t_ep=data.t_ep,
